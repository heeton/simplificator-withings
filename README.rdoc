...
You can handle subscriptions through the API (all devices currently known => Scale + Blood Pressure Monitor)
  user.subscribe_notification('http://foo.bar.com', 'test_subscription')
Specify what devices you are interested  
  user.subscribe_notification('http://foo.bar.com', 'test_subscription', Withings::SCALE, Withings::BLOOD_PRESSURE_MONITOR)
